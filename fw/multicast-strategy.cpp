...
MulticastStrategy::MulticastStrategy(Forwarder& forwarder, const Name& name): Strategy(forwarder), ProcessNackTraits(this), m_retxSuppression(RETX_SUPPRESSION_INITIAL,RetxSuppressionExponential::DEFAULT_MULTIPLIER, RETX_SUPPRESSION_MAX)
{
                                        ...
                                        const shared_ptr<pit::Entry>& pitEntry)
{
  ...
  //std::cout << "> Look up the FIB " << std::endl;
  ...
  for (const auto& nexthop : nexthops) {
    ...
    if (suppressResult == RetxSuppressionResult::SUPPRESS) {
      ...
      //std::cout<<interest << " from=" << ingress << " to=" << outFace.getId() << " suppressed"<<std::endl;
      isSuppressed = true;
      continue;
    }
    ...
    //std::cout << "> Send out interest" << std::endl;
  ...
  if (nEligibleNextHops == 0 && !isSuppressed) {
    ...
    //std::cout<<interest << " from=" << ingress << " noNextHop"<<std::endl;
