                                         ...
                                         const shared_ptr<pit::Entry>& pitEntry)
{
  ...
  if (suppression == RetxSuppressionResult::SUPPRESS) {
    ...
    std::cout<<interest << " from=" << ingress << " suppressed"<<std::endl;
    return;
  }
  ...
  if (suppression == RetxSuppressionResult::NEW) {
    ...
    std::cout<<interest << " from=" << ingress << " newPitEntry-to=" << egress<<std::endl;
  ...
  if (it != nexthops.end()) {
    ...
    std::cout<<interest << " from=" << ingress << " retransmit-unused-to=" << egress<<std::endl;
    return;
  }
  ...
  if (it == nexthops.end()) {
    ...
    std::cout<<interest << " from=" << ingress << "retransmitNoNextHop"<<std::endl;
  }
  else {
    auto egress = FaceEndpoint(it->getFace(), 0);
    this->sendInterest(pitEntry, egress, interest);
    NFD_LOG_DEBUG(interest << " from=" << ingress << " retransmit-retry-to=" << egress);
    std::cout<<interest << " from=" << ingress << " retransmit-retry-to=" << egress<<std::endl;
  }
}
