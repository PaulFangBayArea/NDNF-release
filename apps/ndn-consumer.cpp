...
Consumer::SendPacket()
{
  ...
  std::cout<<"> Consumer node "<< GetNode()->GetId() <<" sends Interest, Interest =  " <<interest->getName()<<std::endl;
...
Consumer::OnData(shared_ptr<const Data> data)
{
  ...
  std::cout<<"> Consumer node "<< GetNode()->GetId() <<" receives Data, Data =  " <<data->getName()<<"; Data is signed by = " << data->getSignature().getValue()<<std::endl;
  //std::cout<<"> Consumer receives DATA for " << seq<<std::endl;
  int hopCount = 0;
