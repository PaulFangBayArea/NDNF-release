...
Consumer2::SendPacket()
{
  ...
  std::cout<<"> Consumer2 node "<< GetNode()->GetId() <<" sends Interest, Interest =  " <<interest->getName()<<std::endl;
...
Consumer2::OnData(shared_ptr<const Data> data)
{
  ...
  std::cout<<"> Consumer2 node "<< GetNode()->GetId() <<" receives Data, Data =  " <<data->getName()<<"; Data is signed by = " << data->getSignature().getValue()<<std::endl;
  //std::cout<<"> Consumer2 receives DATA for " << seq<<std::endl;
  int hopCount = 0;
