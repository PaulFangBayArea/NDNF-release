...
CustomApp::StartApplication()
{
  ...
  ndn::FibHelper::AddRoute(GetNode(), "f/name", m_face, 0);
  // Schedule send of first interest
  //Simulator::Schedule(Seconds(0), &CustomApp::SendInterest, this);
}
...
CustomApp::OnData(std::shared_ptr<const ndn::Data> data)
{
  ...
  std::cout << "NFV node receives Data, Data = " << data->getName() << std::endl;
  //auto& nodePit = GetNode()->GetObject<ndn::L3Protocol>()->getForwarder()->getPit();

  //Name dataName(nodePit);
  // dataName.append(m_postfix);
  // dataName.appendVersion();
  //std::cout<<"> node(" << GetNode()->GetId() << ") receives Interest " << dataName<<std::endl;


  //auto data1 = make_shared<Data>();
  //data1->setName(dataName);
  
  //data1->setFreshnessPeriod(10);
  
 // data1->setContent(make_shared< ::ndn::Buffer>(m_virtualPayloadSize));
