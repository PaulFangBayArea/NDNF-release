...
NS_LOG_COMPONENT_DEFINE("ndn.NFVNode1");

namespace ns3 {
  namespace ndn {
NS_OBJECT_ENSURE_REGISTERED(NFVNode1);
...
NFVNode1::GetTypeId()
{
  static TypeId tid = TypeId("ns3::ndn::NFVNode1").SetParent<App>().AddConstructor<NFVNode1>()
      .AddAttribute("Prefix1", "Prefix, for which NFV node  handle the data", StringValue("/"),
                    MakeNameAccessor(&NFVNode1::m_prefix1), MakeNameChecker())
      .AddAttribute("Prefix2", "Prefix, for which NFV node  handle the data", StringValue("/"),
                    MakeNameAccessor(&NFVNode1::m_prefix2), MakeNameChecker())
      .AddAttribute("Freshness", "Freshness of data packets, if 0, then unlimited freshness",
                    TimeValue(Seconds(0)), MakeTimeAccessor(&NFVNode1::m_freshness),
                    MakeTimeChecker())
      .AddAttribute(
         "Signature",
         "Fake signature, 0 valid signature (default), other values application-specific",
         UintegerValue(0), MakeUintegerAccessor(&NFVNode1::m_signature),
         MakeUintegerChecker<uint32_t>())
      .AddAttribute("KeyLocator",
                    "Name to be used for key locator.  If root, then key locator is not used",
                    NameValue(), MakeNameAccessor(&NFVNode1::m_keyLocator), MakeNameChecker());;
  return tid;
}
...
void
NFVNode1::NFV(){
NS_LOG_INFO("NFV node 1 executes network function f1");
sleep(0.001);
}
void
NFVNode1::SendInterest()
{
  /////////////////////////////////////
  ...
  NS_LOG_INFO("NFV node 1 sends Interest, Interest = " << interest->getName());
...
void
NFVNode1::SendData(std::shared_ptr<const ndn::Data> contentObject)
{ 
  ...
  NS_LOG_INFO("NFV node 1 signs and sends Data, Data = " << data->getName());
...
void
NFVNode1::OnInterest(std::shared_ptr<const ndn::Interest> interest)
{
  //NS_LOG_FUNCTION(this << interest);
  ndn::App::OnInterest(interest);
  NS_LOG_INFO("NFV node 1 receives Interest, Interest = " << interest->getName());
  ...
  if(interest->getName().getPrefix(1)==m_prefix1)
  {
    ...
    Simulator::Schedule(Seconds(0), &NFVNode1::SendInterest, this);
  }
...
void
NFVNode1::OnData(std::shared_ptr<const ndn::Data> data)
{
  if (!m_active)
    return;
