...
main(int argc, char* argv[])
{
  ...
  topologyReader.SetFileName("src/ndnSIM/examples/topologies/ndn-test-edgerouter-caching.txt");
  topologyReader.Read();
  ...
  ndn::FibHelper::AddRoute(InnerER, "/f", Router0, 1); 
  ndn::FibHelper::AddRoute(Router0, "/f", NFV, 1);  
  //ndn::FibHelper::AddRoute(NFV, "/edge", Router1, 1); 
  ndn::FibHelper::AddRoute(Router0, "/edge", InnerER, 1); 
  ...
  ndn::L3RateTracer::InstallAll("ndn-test-edgerouter-L3-trace.txt", Seconds(1));

  ndn::CsTracer::InstallAll("ndn-test-edgerouter-Cs-trace.txt", Seconds(1));

  //printPitFib(nodes);

 
  ndn::AppDelayTracer::InstallAll("ndn-test-edgerouter-app-delays-trace.txt");
